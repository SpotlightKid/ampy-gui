...
class AppWindow(Gtk.ApplicationWindow):
    def __init__(self, *args, **kwargs):
        ...
        filebrowser_box = Gtk.Box(
            orientation=Gtk.Orientation.HORIZONTAL, spacing=6, valign="fill", halign="fill",
        )
        ...
        local_scrolled_win.set_policy(Gtk.PolicyType.AUTOMATIC, Gtk.PolicyType.AUTOMATIC)
        remote_scrolled_win = Gtk.ScrolledWindow()
        remote_scrolled_win.set_policy(Gtk.PolicyType.AUTOMATIC, Gtk.PolicyType.AUTOMATIC)
        ...
        put_button.connect(
            "clicked", self.put_button_clicked, local_treeview, remote_treeview, terminal_buffer,
        )
        get_button.connect(
            "clicked", self.get_button_clicked, local_treeview, remote_treeview, terminal_buffer,
        )
        ...
        mkdir_button.connect(
            "clicked", self.mkdir_button_clicked, remote_treeview, terminal_buffer
        )
        rmdir_button.connect(
            "clicked", self.rmdir_button_clicked, remote_treeview, terminal_buffer
        )
        reset_button.connect(
            "clicked", self.reset_button_clicked, remote_treeview, terminal_buffer
        )
        delete_button.connect(
            "clicked", self.delete_button_clicked, remote_treeview, terminal_buffer
        )
                ...
                if ftype == "f":
                    ...
                    if output.returncode == 0:
                        ...
                        self.set_terminal_text(
                            terminal_buffer, output.stdout.decode("UTF-8") + "\n"
                        )
